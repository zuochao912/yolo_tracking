...
from hough import test_circles
import websockets
import asyncio
...
@torch.no_grad()
async def run(url,weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    async with websockets.connect(url) as websock:
        for path, (imL,imR), (im0Ls,im0Rs), vid_cap, s in dataset:
            ...
            # Process predictions
            for i, det in enumerate(predL):  # left views
                seen += 1
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            if c==32 or c==49:
                                ...
                                if r!=0:
                                    result="in left views : Center is at:    {},    {},the radis is {}".format(left+x,upper+y,r)
                                else:
                                    result="in left views : No circles are detected!"
            ...
            for i, det in enumerate(predR):  # right views
                seen += 1
                if webcam:  # batch_size >= 1
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            if c==32 or c==49:
                                ...
                                x,y,r=test_circles(img_part)
                                if r!=0:
                                    result="in right views : Center is at:{},{},the radis is {}".format(left+x,upper+y,r)
                                else:
                                    result="in right views : No circles are detected!"
                                await websock.send(result)
                                print(result)
                                rightx,righty=x,y
                ...
                # LOGGER.info(f'{s}Done. ({t3 - t2:.3f}s)')
            t4 = time_sync()
            print("3: Circles time:{:.2f}".format(t4))
            if view_img:
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default="0", help='file/dir/URL/glob, 0 for webcam') #0是右目，1是webcam;./videotest/Left2.bmp
...
def main(opt):
    ...
    asyncio.get_event_loop().run_until_complete(run(url,**vars(opt)))
